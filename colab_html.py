def colab_html(file_name, notebookpath="/content/drive/MyDrive/Colab Notebooks/"):
    ...
    # If html with the same name exists, remove it.
    html_file = os.path.join(gdrive_home, file_name.split(".")[0] + ".html")
    ...
    try:
        get_ipython().system(
            "jupyter nbconvert --to html --template lab --output-dir='$gdrive_home' '$notebookpath''$file_name'"
        )
    except:
